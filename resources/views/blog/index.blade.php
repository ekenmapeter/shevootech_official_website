...
<head>
    ...
    <title>Our Blog | Shevootech Technologies Limited</title>
    <meta name="description" content="Read our latest articles on web development, AI, graphic design, animation, and digital business insights.">
    ...
    <style>
        ...
        .blog-card:hover { transform: translateY(-6px); box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); }
...
<body class="antialiased">
    <nav class="fixed w-full z-50 bg-white/80 backdrop-blur-md shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    ...
                    <a href="{{ route('blog.index') }}" class="text-indigo-600 px-3 py-2 font-medium">Blog</a>
        ...
        <div class="mobile-menu hidden md:hidden bg-white shadow-lg">
            <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                ...
                <a href="{{ route('blog.index') }}" class="block px-3 py-2 text-base font-medium text-indigo-600">Blog</a>
    ...
    <section class="pt-32 pb-20 bg-gradient">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center" data-aos="fade-up">
            <h1 class="text-4xl md:text-5xl font-extrabold text-white">Our Blog</h1>
            <p class="mt-4 text-xl text-indigo-100 max-w-3xl mx-auto">Insights, tutorials, and stories from the Shevootech team</p>
        </div>
    </section>
    ...
    <section class="py-16 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @php
                ...
                $categories = ['dev' => 'Development', 'business' => 'Business', 'general' => 'General'];
            @endphp
            ...
            <div class="flex flex-wrap justify-center gap-3 mb-12" data-aos="fade-up">
                <a href="{{ route('blog.index') }}"
                   ...
                   {{ !$currentCategory ? 'bg-indigo-600 text-white' : 'bg-white text-gray-700 hover:bg-indigo-50 border border-gray-200' }}">
                    All
                </a>
                @foreach($categories as $key => $label)
                <a href="{{ route('blog.index', ['category' => $key]) }}"
                   class="px-5 py-2 rounded-full text-sm font-medium transition duration-300
                   {{ $currentCategory === $key ? 'bg-indigo-600 text-white' : 'bg-white text-gray-700 hover:bg-indigo-50 border border-gray-200' }}">
                    {{ $label }}
                </a>
                @endforeach
            </div>
            ...
            <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                ...
                <a href="{{ route('blog.show', $post->slug) }}" class="blog-card bg-white rounded-xl shadow-md overflow-hidden border border-gray-100 block" data-aos="fade-up" data-aos-delay="{{ ($index + 1) * 100 }}">
                    @if($post->cover_image)
                    <div class="h-48 overflow-hidden">
                        <img loading="lazy" src="{{ $post->cover_image }}" alt="{{ $post->title }}" class="w-full h-full object-cover">
                    </div>
                    @else
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="text-xs text-gray-400">
                                <i class="far fa-calendar-alt mr-1"></i>{{ $post->published_at->format('M d, Y') }}
                            </span>
                            @endif
                        </div>
                        <h3 class="text-lg font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">{{ $post->title }}</h3>
                        <p class="mt-2 text-sm text-gray-500 leading-relaxed">{{ Str::limit($post->excerpt ?? strip_tags($post->body), 120) }}</p>
                        @if($post->author)
                        <div class="mt-4 flex items-center">
                            ...
                            <div class="ml-3">
                                <p class="text-sm font-medium text-gray-900">{{ $post->author->name ?? $post->author }}</p>
                            </div>
                        </div>
                        @endif
                    </div>
            ...
            <div class="text-center py-20">
                ...
                <p class="text-2xl text-gray-400">No blog posts yet.</p>
