...
<head>
    ...
    <title>Store | Shevootech Technologies Limited</title>
    <meta name="description" content="Browse our marketplace of scripts, kits, and digital products.">
    ...
    <style>
        ...
        .product-card:hover { transform: translateY(-6px); box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); }
...
<body class="antialiased">
    <nav class="fixed w-full z-50 bg-white/80 backdrop-blur-md shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    ...
                    <a href="{{ route('store.index') }}" class="text-indigo-600 px-3 py-2 font-medium">Store</a>
                    @auth
        ...
        <div class="mobile-menu hidden md:hidden bg-white shadow-lg">
            <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                ...
                <a href="{{ route('store.index') }}" class="block px-3 py-2 text-base font-medium text-indigo-600">Store</a>
                @auth
    ...
    <section class="pt-32 pb-20 bg-gradient">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center" data-aos="fade-up">
            <h1 class="text-4xl md:text-5xl font-extrabold text-white">Digital Store</h1>
            <p class="mt-4 text-xl text-indigo-100 max-w-3xl mx-auto">Premium scripts, kits, and digital products for your projects</p>
        </div>
    </section>
    ...
    <section class="py-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
                ...
                <a href="{{ route('store.show', $product->slug) }}" class="product-card bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden block" data-aos="fade-up" data-aos-delay="{{ ($index + 1) * 100 }}">
                    @if(isset($product->thumbnail) && $product->thumbnail)
                    <div class="h-48 overflow-hidden bg-gray-100">
                        <img loading="lazy" src="{{ $product->thumbnail }}" alt="{{ $product->title }}" class="w-full h-full object-cover">
                    </div>
                    @else
                    <div class="h-48 bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center">
                        <i class="fas fa-box-open text-white text-4xl opacity-50"></i>
                    </div>
                    @endif
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            @endif
                            @if($product->price)
                            <span class="text-lg font-bold text-indigo-600">₦{{ number_format($product->price) }}</span>
                            @endif
                        </div>
                        <h3 class="text-lg font-semibold text-gray-900">{{ $product->title }}</h3>
                        <p class="mt-2 text-sm text-gray-500 leading-relaxed">{{ Str::limit($product->description, 100) }}</p>
                    </div>
                </a>
                @endforeach
            </div>
            ...
            <div class="text-center py-20">
                ...
                <p class="text-2xl text-gray-400">No products in the store yet.</p>
            </div>
            @endif
        </div>
    </section>
