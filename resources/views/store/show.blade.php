...
<head>
    ...
    <title>{{ $product->title }} | Shevootech Store</title>
...
<body class="antialiased">
    ...
    <section class="pt-32 pb-12 bg-gradient">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <a href="{{ route('store.index') }}" class="text-indigo-200 hover:text-white transition-colors inline-flex items-center mb-6">
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-200 text-blue-900">
                    <i class="fas fa-code mr-1"></i> Script
                </span>
                @elseif($product->type === 'kit')
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-200 text-purple-900">
                    <i class="fas fa-puzzle-piece mr-1"></i> Kit
                </span>
                @else
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-200 text-gray-900">
                    {{ ucfirst($product->type) }}
                </span>
                @endif
            </div>
            <h1 class="text-3xl md:text-4xl font-extrabold text-white" data-aos="fade-up">{{ $product->title }}</h1>
        </div>
    </section>
                ...
                <div class="lg:col-span-2" data-aos="fade-right">
                    ...
                    <div class="grid gap-4 mb-8">
                        ...
                        <div class="rounded-xl overflow-hidden shadow-md">
                            <img loading="lazy" src="{{ $screenshot }}" alt="{{ $product->title }}" class="w-full h-auto">
                        </div>
                        @endforeach
                    </div>
                    @elseif($product->thumbnail)
                    <div class="rounded-xl overflow-hidden shadow-md mb-8">
                        <img loading="lazy" src="{{ $product->thumbnail }}" alt="{{ $product->title }}" class="w-full h-auto">
                    </div>
                    @else
                    <div class="rounded-xl overflow-hidden shadow-md bg-gradient-to-br from-indigo-500 to-purple-600 p-20 text-center mb-8">
                        <i class="fas fa-box-open text-6xl text-white opacity-50"></i>
                    </div>
                    @endif
                ...
                <div class="mt-8 lg:mt-0 lg:col-span-1" data-aos="fade-left">
                    <div class="bg-gray-50 rounded-xl p-8 shadow-md sticky top-28">
                        <div class="text-center border-b border-gray-200 pb-6 mb-6">
                            @if($product->price)
                            <p class="text-4xl font-bold text-indigo-600">₦{{ number_format($product->price) }}</p>
                            @else
                        ...
                        @if($product->demo_url)
                        <a href="{{ $product->demo_url }}" target="_blank" class="w-full inline-flex items-center justify-center px-6 py-3 border border-gray-300 text-base font-medium rounded-full text-gray-700 bg-white hover:bg-gray-50 transition duration-300 mb-3">
                            <i class="fas fa-external-link-alt mr-2"></i> View Demo
                        </a>
                        @endif

                        @auth
                        <form method="POST" action="{{ route('checkout.create', $product) }}">
                            @csrf
                            <button type="submit" class="w-full inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-full text-white bg-indigo-600 hover:bg-indigo-700 transition duration-300 shadow-md">
                                <i class="fas fa-shopping-cart mr-2"></i> Buy Now - ₦{{ number_format($product->price) }}
                            </button>
                        </form>
                        @else
                        <a href="{{ route('login') }}" class="w-full inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-full text-white bg-indigo-600 hover:bg-indigo-700 transition duration-300 shadow-md">
