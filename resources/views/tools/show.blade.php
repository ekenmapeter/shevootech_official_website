...
<head>
    ...
    <title>{{ $tool->title }} | Shevootech Tools</title>
    <meta name="description" content="{{ Str::limit($tool->description, 160) }}">
...
<body class="antialiased">
    ...
    <section class="pt-32 pb-12 bg-gradient">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <a href="{{ route('tools.index') }}" class="text-indigo-200 hover:text-white transition-colors inline-flex items-center mb-6">
                <i class="fas fa-arrow-left mr-2"></i> Back to Tools
            </a>
            <div class="flex items-center gap-4 mb-4">
                <div class="flex items-center justify-center h-16 w-16 rounded-xl bg-white/20 text-white text-3xl">
                ...
                <div>
                    ...
                    @else
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-200 text-yellow-900">
                        <i class="fas fa-crown mr-1"></i> Premium
                    </span>
                    @endif
                </div>
            </div>
            <h1 class="text-3xl md:text-4xl font-extrabold text-white" data-aos="fade-up">{{ $tool->title }}</h1>
        </div>
    </section>

    <section class="py-16 bg-white">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-gray-50 rounded-xl p-8 md:p-12" data-aos="fade-up">
                @if($tool->is_free)
                ...
                @else
                @php
                    $hasPurchased = auth()->check() && $tool->product && auth()->user()->hasPurchased($tool->product);
                @endphp
                ...
                <div class="text-center py-12">
                    ...
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">Premium Tool</h2>
                    <p class="text-gray-500 max-w-lg mx-auto mb-8">
                        This is a premium tool. Purchase it to unlock full access and start using it immediately.
                    </p>
                    @auth
                    @if($tool->product)
                    <a href="{{ route('store.show', $tool->product->slug) }}" class="inline-flex items-center px-8 py-3 border border-transparent text-base font-medium rounded-full text-white bg-indigo-600 hover:bg-indigo-700 transition duration-300 shadow-md">
                        <i class="fas fa-shopping-cart mr-2"></i> Buy to Unlock
                    </a>
                    @endif
                    @else
                    <a href="{{ route('login') }}" class="inline-flex items-center px-8 py-3 border border-transparent text-base font-medium rounded-full text-white bg-indigo-600 hover:bg-indigo-700 transition duration-300 shadow-md">
